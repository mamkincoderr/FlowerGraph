...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._channel_panel.sig_visibility.connect(self._on_channel_visibility)
        ...
        self._nav_bar.pause_toggled.connect(self._on_pause_toggled)
        ...
        self._plot_area.selection_expand_requested.connect(self._expand_selection)
        ...
        self._act_follow = QAction('Следить за записью', self)
        self._act_follow.setCheckable(True)
        self._act_follow.setChecked(True)
        self._act_follow.setToolTip('Прокручивать экран к последним данным')
        ...
        self._act_space = QAction(self)
        self._act_space.setShortcut(QKeySequence(Qt.Key_Space))
        self._act_space.setShortcutContext(Qt.ShortcutContext.WindowShortcut)
        self._act_space.triggered.connect(self._on_space)
        self.addAction(self._act_space)
    ...
    def _build_menu(self):
        ...
        m_rec.addSeparator()
        self._act_start = self._action(tr('start'), self._on_start)
    ...
    def _build_toolbar(self):
        ...
        # Стоп остаётся в меню и на большой кнопке внизу.
        ...
        self._act_auto_y.setToolTip('Подгонять вертикальную шкалу под все каналы на экране')
        self._act_auto_y.triggered.connect(
            lambda c: self._plot_area.set_y_autorange(c)
        )
        tb.addAction(self._act_auto_y)

        self._act_markers = QAction('Маркеры', self)
        self._act_markers.setCheckable(True)
        self._act_markers.setToolTip('Два маркера: интервал времени и разность значений')
        ...
        tb.addWidget(QLabel(' Каналы: '))
        act_ov   = self._action('Нули', self._on_y_overlay)
        act_ov.setToolTip('Поставить ноль всех каналов на одну линию')
        act_dist = self._action('Разнести', self._on_y_distribute)
        act_dist.setToolTip('Разложить каналы друг под другом')
        act_auto = self._action('По каналам', self._on_y_auto)
        act_auto.setToolTip('Подогнать масштаб каждого канала отдельно')
        act_rst  = self._action('Сброс', self._on_y_reset)
        act_rst.setToolTip('Вернуть исходный масштаб и смещение каналов')
        for a in (act_ov, act_dist, act_auto, act_rst):
            tb.addAction(a)
        tb.addSeparator()

        act_yzin  = self._action('Крупнее', self._on_y_zoom_in)
        act_yzin.setToolTip('Увеличить активный канал по вертикали')
        act_yzout = self._action('Мельче', self._on_y_zoom_out)
        act_yzout.setToolTip('Уменьшить активный канал по вертикали')
    ...
    def _on_stop(self):
        ...
        # Пауза — защёлка пользователя. Стоп её не отпускает и не сдвигает кадр.
        paused = self._nav_bar.is_paused()
        # Стоп не двигает кадр и не меняет цену деления — ни с паузой, ни без.
        vb_range = self._plot_area._plot.plotItem.getViewBox().viewRange()
        saved_view = (
            float(vb_range[0][0]), float(vb_range[0][1]),
            self._plot_area.time_div_idx,
            float(vb_range[1][0]), float(vb_range[1][1]),
        )
        saved_scales = np.array(self._plot_area._scales, copy=True)
        saved_offsets = np.array(self._plot_area._offsets, copy=True)
        saved_auto_y = self._act_auto_y.isChecked()
        ...
        self._nav_bar.set_paused(paused)
        self._pause_arm = False
        if was_recording and has_data:
            self._plot_area.restore_channel_gains(saved_scales, saved_offsets)
            self._plot_area.restore_paused_view(
                saved_view[0], saved_view[1], saved_view[2],
            )
            self._plot_area._refresh()
            self._plot_area.restore_y_range(saved_view[3], saved_view[4], saved_auto_y)
            for i in range(min(len(saved_scales), len(self._channel_panel._rows))):
                self._channel_panel.update_scale_offset(
                    i, float(saved_scales[i]), float(saved_offsets[i]),
                )
            self._act_auto_y.blockSignals(True)
            self._act_auto_y.setChecked(saved_auto_y)
    ...
    def _on_channel_visibility(self, idx: int, visible: bool):
        self._plot_area.set_channel_visible(idx, visible)
        while len(self._channel_visible) <= idx:
            self._channel_visible.append(True)
        self._channel_visible[idx] = bool(visible)

    def _restore_channel_visibility(self, n: int):
        """Старт записи заново собирает строки. Скрытые каналы остаются скрытыми."""
        saved = self._channel_visible
        if len(saved) != n:
            self._channel_visible = [True] * n
            return
        for i, visible in enumerate(saved):
            self._plot_area.set_channel_visible(i, visible)
            if i < len(self._channel_panel._rows):
                self._channel_panel._rows[i].set_visible(visible)
    ...
    def _finalize_block(self, keep_view: bool = False):
        ...
        self._display_block(self._current_block_idx, fit=not keep_view)
        self._refresh_block_list()
        self._update_title()
        # Без паузы показать весь блок. С паузой кадр остаётся там, где его оставили.
        if not keep_view:
            self._on_show_block()
    ...
    def _display_block(self, idx: int, *, fit: bool = True):
        ...
        self._restore_channel_visibility(block.n_channels)
        self._refresh_block_list()
    ...
    def _on_pause_toggled(self, paused: bool):
        """Пауза останавливает только прокрутку. Запись не трогает."""
        self._plot_area.set_following(not paused)
    ...
    def _on_following_changed(self, following: bool):
        ...
        if self._pause_hold:
            return
        self._nav_bar.set_paused(not following)
    ...
    def _on_selection_changed(self, t0: float, t1: float, n: int):
        if n == 0:
            ...
            dt = t1 - t0
            self._lbl_sel.setText(
                f'▐ Выделение: {dt:.4f} с  ·  {n:,} отсч.  ·  Пробел — на экран'
            )
    ...
    def _on_space(self):
        """Пробел: выделенный кусок на весь экран, иначе старт записи."""
        if self._expand_selection():
            return
        if self._state == AppState.IDLE:
            self._on_start()

    def _expand_selection(self) -> bool:
        """Раскрыть выделение на ширину экрана. С Авто Y — ещё и по вертикали."""
        t_div = self._plot_area.expand_selection(self._act_auto_y.isChecked())
        if t_div is None:
            return False
        self._nav_bar.update_scale_label(fmt_time_div(t_div))
        return True
